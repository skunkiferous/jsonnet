...
local test_str2Lines() =
	assert spr.str2Lines('') == [];
	assert spr.str2Lines('x') == ['x'];
	assert spr.str2Lines('y\n') == ['y'];
	assert spr.str2Lines('y\nz') == ['y', 'z'];
	true;

local test_str2TSV() =
	assert spr.str2TSV('') == [];
	assert spr.str2TSV('x') == [['x']];
	assert spr.str2TSV('y\n') == [['y']];
	assert spr.str2TSV('y\nz') == [['y'], ['z']];
	assert spr.str2TSV('x\ty\nz') == [['x','y'], ['z']];
	assert spr.str2TSV('x\t\nz') == [['x',''], ['z']];
	assert spr.str2TSV('\ty\nz') == [['','y'], ['z']];
	true;
...
local test_safeParseJSON() =
	assert spr.safeParseJSON("test",0,"f","") == {"errors": [{"ERROR": "'JSON' value '\"\"' is not valid",
		"Field": "f", "Index": "0", "Source": "test"}], "result": null};
	
	assert spr.safeParseJSON("test",0,"f","null") == { result: null, errors: [] };
	assert spr.safeParseJSON("test",0,"f","true") == { result: true, errors: [] };
	assert spr.safeParseJSON("test",0,"f","false") == { result: false, errors: [] };
	assert spr.safeParseJSON("test",0,"f","123.456") == { result: 123.456, errors: [] };
	assert spr.safeParseJSON("test",0,"f",'""') == { result: '', errors: [] };
	...
	assert spr.safeParseJSON("test",0,"f","{}") == { result: {}, errors: [] };
	assert spr.safeParseJSON("test",0,"f",'{"f":null}') == { result: {"f":null}, errors: [] };
	assert spr.safeParseJSON("test",0,"f",'{"f":true}') == { result: {"f":true}, errors: [] };
	assert spr.safeParseJSON("test",0,"f",'{"f":false}') == { result: {"f":false}, errors: [] };
	assert spr.safeParseJSON("test",0,"f",'{"f":123.456}') == { result: {"f":123.456}, errors: [] };
	assert spr.safeParseJSON("test",0,"f",'{"f":""}') == { result: {"f":''}, errors: [] };
	
	# TODO Validate bad JSON too
	true;

local test_safeParse() =
	assert spr.safeParse("test",0,"f","boolean","true") == { result: true, errors: [] };
	assert spr.safeParse("test",0,"f","number","123.456") == { result: 123.456, errors: [] };
	assert spr.safeParse("test",0,"f","int","42") == { result: 42, errors: [] };
	assert spr.safeParse("test",0,"f","hex","0xFF") == { result: 255, errors: [] };
	assert spr.safeParse("test",0,"f","json",'{"json":true}') == { result: {"json":true}, errors: [] };
	assert spr.safeParse("test",0,"f","string","xxx") == { result: "xxx", errors: [] };
	
	assert spr.safeParse("test",0,"f","boolean","Z") == {"errors":
		[{"ERROR": "'boolean' value 'Z' is not valid", "Field": "f", "Index": "0", "Source": "test"}],
		"result": null};
	assert spr.safeParse("test",0,"f","number","Z") == {"errors":
		[{"ERROR": "'number' value 'Z' is not valid", "Field": "f", "Index": "0", "Source": "test"}],
		"result": null};
	assert spr.safeParse("test",0,"f","int","Z") == {"errors":
		[{"ERROR": "'integer' value 'Z' is not valid", "Field": "f", "Index": "0", "Source": "test"}],
		"result": null};
	assert spr.safeParse("test",0,"f","hex","Z") == {"errors":
		[{"ERROR": "'hex' value 'Z' is not valid", "Field": "f", "Index": "0", "Source": "test"}],
		"result": null};
	assert spr.safeParse("test",0,"f","json","") == {"errors":
		[{"ERROR": "'JSON' value '\"\"' is not valid", "Field": "f", "Index": "0", "Source": "test"}],
		"result": null};
	# "string" is always valid
	
	assert spr.safeParse("test",0,"f","complex","zzz") == {"errors":
...
local test_tsv2TypedTSV() =
	assert spr.tsv2TypedTSV("test", spr.str2TSV('x\nz')) == {"errors": [ ], "result": [["x"], ["z"]]};
	assert spr.tsv2TypedTSV("test", spr.str2TSV('x:int\n0')) == {"errors": [ ], "result": [["x:int"], [0]]};
	assert spr.tsv2TypedTSV("test",
